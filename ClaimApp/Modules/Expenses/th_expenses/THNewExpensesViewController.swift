//
//  THNewExpensesViewController.swift
//  ClaimApp
//
...
let th_normalSection = 0
let th_amountSection = 1
let th_captureSection = 2
let th_paymentSection = 3
let th_costItemSection = 4

enum THExpensesTextFieldTag: Int {
    case category = 100
    case merchant
    case carplate
    case vendor
...
enum THExpensesSectionType: Int {
    case normal = 0
    case amount
    case capture
}

class THNewExpensesViewController: CABaseViewController {
    ...
    @IBOutlet weak var personalHeightConstraint: NSLayoutConstraint!
    ...
    var selectedCurrencyInfo:CACurrencyInfo?

    var selectedVendor: String = ""
    
    var vendorInfor: CADropdownItem?
    var hasPaymentSelectedVendor = false
    var hasVendorSelectedVendor = false
    var isFirstForPaymentToSelect = true
    ...
    lazy var footView: UIView = {
        ...
        button.tag = 100
        button.addTarget(self, action: #selector(THNewExpensesViewController.submitExpense), for: .touchUpInside)
...
//        header.addSubview(submitMark)
    ...
    override func viewDidLoad() {
        ...
        if AppDataHelper.countryCode == .TH  || notSubmitData != nil {
            personalButton.isHidden = true
            inventoryButton.isHidden = personalButton.isHidden
            personalHeightConstraint.constant = 0
            tableHeadView.height = 34
        } else {
            personalButton.isHidden = false
            personalHeightConstraint.constant = 56
            inventoryButton.isHidden = personalButton.isHidden
        }
    }
    
    private func addObserver() {
        NotificationCenter.default.addObserver(self, selector: #selector(THNewExpensesViewController.draftDescriptionDidChanged(noti:)), name: DraftDescriptionIsChanged, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(THNewExpensesViewController.dropListDidBeginTpyeIn(noti:)), name: SearchFieldDidBeginSearch, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(THNewExpensesViewController.lastOptionsDidChanged(noti:)), name: PaymentChooseLastOption, object: nil)
        
        tableView.addObserver(self, forKeyPath: "contentOffset", options: [.new], context: nil)
        
        NotificationCenter.default.addObserver(self, selector: #selector(THNewExpensesViewController.didSelectCurrency(notification:)), name: DraftDidChooseCurrency, object: nil)
        
        NotificationCenter.default.addObserver(self, selector: #selector(THNewExpensesViewController.amountUpdate(notification:)), name: AmountDidUpdateToVCNotification, object: nil)
        
    }
    
    @objc private func amountUpdate(notification: Notification) {
        let model = notification.object as? [String:Any]
        
        var lastTotalNet = 0.00
        var lastTotalFax = 0.00
        var lastTotal = 0.00
        ...
        for sections in datas {
            for sectionData in sections {
                ...
                if let itemData = sectionData as? ItemsFormFieldData {
                    let netItem = itemData.items[0]
                    let taxItem = itemData.items[1]
                    if let net = netItem.text {
                        let tempNet = CFStringGetDoubleValue(net as CFString?)
                        lastTotalNet += tempNet

                    }
                    if let _ = taxItem.text {
                        let tempFax = CFStringGetDoubleValue(taxItem.text as CFString?)
                        lastTotalFax += tempFax
                    }
                }
            }
        }
        lastTotal = lastTotalNet + lastTotalFax
        
        if let costData = datas.last?.last as? THCostItemData {
            costData.subtotal = lastTotalNet == 0 ? "-" : String(format: "%.2f", lastTotalNet).conversionOfDigital(separator: ",")
            costData.totalVAT = lastTotalFax == 0 ? "-" : String(format: "%.2f", lastTotalFax).conversionOfDigital(separator: ",")
            costData.grandTotal = lastTotal == 0 ? "-" : String(format: "%.2f", lastTotal).conversionOfDigital(separator: ",")
            tableView.reloadSections([datas.count-1], with: .none)
        }
    }
    ...
    @objc private func didSelectCurrency(notification: Notification) {
        ...
        if let info = self.currencyInfo , info.currency_code == model.currency_code {
            self.selectedCurrencyInfo = nil
            return
        }
        self.selectedCurrencyInfo = model
        
    }
    
    private func hasSelectedCurrency() -> Bool {
        if self.selectedCurrencyInfo != nil {
            return true
        }
        return false
    }
...
//        if isFirstForPaymentToSelect {
//            return
//        }
        
        guard AppDataHelper.countryCode == .TH else {
            return
        }
        guard let data = noti.object as? DropdownFormFieldData else {return}
        
        let selectedData = data.selectedIndex.map{data.items[$0]}

        if selectedData?.name == "Vendor" {
            hasPaymentSelectedVendor = true
            var paymentSection = self.datas[th_paymentSection]
            
            if paymentSection.count > 3 {
                paymentSection.removeLast()
                paymentSection.removeLast()
                paymentSection.removeLast()
                if hasPaymentSelectedVendor , let vendor = vendorInfor {
                    insertBankField(item: vendor)
                    return
                }
            }else if paymentSection.count == 3 {
                return
            }else if paymentSection.count > 1{
                paymentSection.removeLast()
                self.datas[th_paymentSection] = paymentSection
                self.tableView.reloadSections([th_paymentSection], with: .none)
                if hasPaymentSelectedVendor , let vendor = vendorInfor {
                    insertBankField(item: vendor)
                    return
                }
            } else if paymentSection.count == 1,hasPaymentSelectedVendor,let vendor = vendorInfor {
                insertBankField(item: vendor)
                return
            } else {
                return
            ...
            hasPaymentSelectedVendor = false
            insertPayeeNameField()
        }
        
    }
    
    private func insertPayeeNameField() {
        var paymentSection = self.datas[th_paymentSection]
        if paymentSection.count > 1 {
            let data = paymentSection[1]
            if data.isKind(of: SearchFormFieldData.self) {
                if paymentSection.count > 3 {
                    paymentSection.removeLast()
                    paymentSection.removeLast()
                    self.datas[th_paymentSection] = paymentSection
        ...
        let formData7 = SearchFormFieldData(localizedTitleKey: "")
        formData7.placeholder = "Enter payee name"
        paymentSection.insert(formData7, at: 1)
        self.datas[th_paymentSection] = paymentSection
        self.tableView.reloadSections([th_paymentSection], with: .none)
    }
...
//        topSpace.constant = 56
    ...
    private func registerCell() {
        ...
        tableView.register(UINib.init(nibName: "THNewExpensesAmountCell", bundle: nil), forCellReuseIdentifier: tableAmountIdentifierCell)
        ...
        tableView.register(UINib.init(nibName: "THPaymentCurrencyCell", bundle: nil), forCellReuseIdentifier: tablePaymentCurrencyIdentifierCell)
        
        tableView.register(UINib.init(nibName: "THCostTotalCell", bundle: nil), forCellReuseIdentifier: tableCostItemIdentifierCell)
    ...
    private func currentUsedCurrency() -> CACurrencyInfo? {
        if let currency = self.selectedCurrencyInfo {
            return currency
        } else if let currency = self.currencyInfo {
            return currency
        } else {
            return dropInfo.currencies?.default
        }
    }
    ...
    func initResourceData() {
        ...
        paymentCurrency.currency = currentUsedCurrency()
        ...
        if let draftInfo = draftInfo {
            ...
            formData4.currencyInfo = currentUsedCurrency()
        ...
        if let draftInfo = draftInfo {
            ...
            if draftInfo.payee?.value == 0 {
                formData6.selectedIndex = 0
                datas.append([formData6])
            } else {
                formData6.selectedIndex = 1
                let formData7 = SearchFormFieldData(localizedTitleKey: "")
                formData7.placeholder = "Enter payee name"
                formData7.text = draftInfo.payee?.title
                
                if let payee = draftInfo.payee , let bankName = payee.bank?.display_name , let bankNo = payee.bank?.account_no {
                    ...
                    formData1_2.text = bankNo
                    
                    datas.append([formData6,formData7,formData1_1,formData1_2])
                } else {
                    datas.append([formData6,formData7])
                }
            }
        ...
        let formData8 = THCostItemData(localizedTitleKey: "")
        formData8.currency = currentUsedCurrency()
        datas.append([formData8])
        
        if let draftInfo = draftInfo {
            if let subtotal = draftInfo.total?.amount.doubleValue {
                formData8.subtotal = String(format: "%.2f", subtotal)
            }
            if let subtotal = draftInfo.total?.tax_amount.doubleValue {
                formData8.totalVAT = String(format: "%.2f", subtotal)
            }
            if let subtotal = draftInfo.total?.gross_amount.doubleValue {
                formData8.grandTotal = String(format: "%.2f", subtotal)
            }
        }
...
//        }
        ...
        if let _ = draftInfo {
            let tipVc = LNOfferExitTipViewController(target: Target(target: self, selector: #selector(THNewExpensesViewController.dismissAction2(obj:))), selectType: self.expensesType.rawValue)
    ...
    @IBAction func cancelButtonClicked(_ sender: UIButton) {
        ...
        let tipVc = LNOfferExitTipViewController(target: Target(target: self, selector: #selector(THNewExpensesViewController.dismissAction(obj:))))
...
extension THNewExpensesViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let photoData = fieldData as? PhotoFormFieldData, photoData.images.count > 0 {
          ...
          let cell = tableView.dequeueReusableCell(withIdentifier: tableAmountIdentifierCell, for: indexPath) as! THNewExpensesAmountCell
          ...
          itemData.currency = currentUsedCurrency()
          cell.bind(data: itemData)
          cell.selectionStyle = .none
          return cell
...
extension THNewExpensesViewController: UITableViewDelegate {
    
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if indexPath.section == th_costItemSection , let model = self.selectedCurrencyInfo {
            NotificationCenter.default.post(name: DraftDidChooseCurrencyForTotalCell, object: model)
            self.selectedCurrencyInfo = nil
        }
    }
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if indexPath.section == th_amountSection {
            if let data = datas[amountSection][indexPath.row] as? ItemsFormFieldData {
                return data.rowHeight - (data.isPersonal ? 36+10 : 0)
            }
        } else if indexPath.section == th_captureSection {
            
            if let formData = datas[2].first as? PhotoFormFieldData, formData.images.count > 0 {
                return 160 + 16
            }
            return 0
        } else if indexPath.section == th_paymentSection {
            if let data = datas[indexPath.section][indexPath.row] as? SearchFormFieldData {
                if data.title.isEmpty {
                    return 69 - 22
                }
                return 69
            }
        } else if indexPath.section == th_costItemSection {
            return 215
        }
    ...
    func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
        ...
        switch section {
        case th_normalSection:
            return 0.01
        case th_amountSection:
            return 0
        case th_captureSection:
            if let formData = datas[2].first as? PhotoFormFieldData, formData.images.count > 0 {
                return 0
            }
            return 36 + 8
...
extension THNewExpensesViewController: TLPhotosPickerViewControllerDelegate {
    ...
    func dismissPhotoPicker(withTLPHAssets: [TLPHAsset]) {
        ...
        case .photo:
            ...
            self.tableView.reloadSections([th_captureSection], with: .none)
...
//MARK: - selectors
extension THNewExpensesViewController {
...
//            self.tableViewSpace.constant = 23
    ...
    @objc private func didShowResult(obj: Any) {
        ...
        if let cell = selectingCell as? CANewExpensesNormalCell {
            if let data = cell.data as? SearchFormFieldData {
                ...
                selectedVendor = item.title
                if AppDataHelper.countryCode == .TH {
                    hasVendorSelectedVendor = false
                    if includeType == .payee {
                        insertBankField(item: item)
                    } else if includeType == .vendor {
                        hasVendorSelectedVendor = true
                        vendorInfor = item
                        if hasPaymentSelectedVendor && hasVendorSelectedVendor {
                            insertBankField(item: item)
                        }
                    }
                }
        ...
        }else if let cell = selectingCell as? THNewExpensesAmountCell {
    ...
    private func updateBankInfo(section: [FormFieldData],item: CADropdownItem) {
        var bankNameIndex = 1
        var bankNoIndex = 2
        if section.count == 4 {
            bankNameIndex = 2
        ...
        let bankForm = section[bankNameIndex]
        let bankNoFrom = section[bankNoIndex]
        bankForm.text = item.bank?.display_name
        bankNoFrom.text = item.bank?.account_no
        self.datas[th_paymentSection] = section
        self.tableView.reloadSections([th_paymentSection], with: .none)
    }
    
    private func insertBankField(item: CADropdownItem) {
        if self.datas.count > th_paymentSection {
            var section0 = self.datas[th_paymentSection]
            if section0.count > 2 {
                if section0.count == 3 , selectingIncludeType == .vendor {
                    updateBankInfo(section: section0, item: item)
                } else if section0.count == 4, selectingIncludeType == .vendor {
                    section0.remove(at: 1)
                    updateBankInfo(section: section0, item: item)
                } else if section0.count == 4, selectingIncludeType == .payee {
                    updateBankInfo(section: section0, item: item)
                }
                return
            }
        ...
        formData1_1.editable = false
        formData1_1.placeholder = .localized("new_expenses_title_bank_placeholder", comment: "")
        formData1_1.text = item.bank?.display_name
        
        let formData1_2 = TextFormFieldData(localizedTitleKey: .localized("new_expenses_title_bank_no.", comment: ""))
        formData1_2.editable = false
        formData1_2.placeholder = .localized("new_expenses_title_bank_no._placeholder", comment: "")
        formData1_2.text = item.bank?.account_no

        if self.datas.count > th_paymentSection {
            var section0 = self.datas[th_paymentSection]
            if section0.count == 2 , hasVendorSelectedVendor {
                section0.removeLast()
            }
            if section0.count > 1 {
                section0.insert(formData1_1, at: 2)
                section0.insert(formData1_2, at: 3)
            } else if section0.count > 0 {
                section0.insert(formData1_1, at: 1)
                section0.insert(formData1_2, at: 2)
            }
            
            self.datas[th_paymentSection] = section0
            self.tableView.reloadSections([th_paymentSection], with: .none)
        }
    ...
    @objc func draftDescriptionDidChanged(noti: Notification) {
        ...
        if  let obj = noti.object as? [String:Any], let heightDifference = obj["heightDifference"] as? CGFloat, let cell = obj["cell"] as? THNewExpensesAmountCell {
...
//        }
    ...
    @objc private func addNewItem(_ sender: UIButton) {
        ...
        formData.currencyInfo = currentUsedCurrency()
    ...
    @objc private func removeAmount(_ sender: UIButton) {
        ...
        amountUpdate(notification: Notification(name: AmountDidUpdateToVCNotification))
    }
    ...
    @objc func dropListDidBeginTpyeIn(noti: Notification) {
        ...
        if let c = obj["cell"] as? CANewExpensesNormalCell {
            ...
            selectingCell = c
            if c.data.placeholder == "Enter payee name" {
                selectingIncludeType = .payee
            }
        }else if let c = obj["cell"] as? THNewExpensesAmountCell {
...
//        params["payee_id"] = ""
        ...
        if expensesType == .personal {
            ...
            if let invoiceNo = datas[0][index].text, invoiceNo.count > 0 {
            ...
            if let fieldData = datas[0][index] as? DateFormFieldData, let value = fieldData.text, value.count > 0 {
        ...
        if datas[3].count > 1 {
            if let formData = datas[3][1] as? SearchFormFieldData,let data = formData.selectedItem {
                params["payee_id"] = data.value
            }
        } else if datas[3].count == 1 {
        ...
        for sectionData in datas[1] {
            ...
            if let itemData = sectionData as? ItemsFormFieldData {
                ...
                if let fieldData = itemData.items[2] as? SearchFormFieldData, let title = fieldData.selectedItem?.title, title.count > 0 ,let value = fieldData.selectedItem?.value, value.count > 0 {
                    ...
                    itemDic["owned_inventory_id"] = value
                }
